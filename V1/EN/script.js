...
(function(){
  ...
  if(!el||!bar)return;
  var p=0,msgs=['LOADING','BUILDING','RENDERING','READY'];
...
(function(){
  ...
  if(isWeekday&&inHours){
    ...
    txt.textContent='Available now — Landgraaf, Netherlands';
  } else if(almostOpen||almostClose){
    dot.classList.add('status-soon');
    txt.textContent=almostOpen?'Available soon — opens at 09:00':'Closing soon — available until 17:30';
  } else {
...
(function(){
  ...
  form.addEventListener('submit',function(e){
    ...
    function doSuccess(){
      ...
      showToast('Message sent. Back to you within one business day.');
    }
    ...
    fetch('https://formspree.io/f/'+FORMSPREE_ID,{
      ...
      if(data.ok||data.next){doSuccess();}
        ...
        showToast('Something went wrong. Send an email to ibemotionmedia@gmail.com',true);
      }
    }).catch(function(){
      if(btn){btn.disabled=false;var bt=btn.querySelector('.btn-t');if(bt)bt.textContent='Verstuur aanvraag';}
      showToast('No connection. Please try again or email directly.',true);
    });
  });
})();
...
(function(){
  ...
  function updateCalc(){
    if(!selected.length){
      daysOut.textContent='—';servOut.textContent='Select at least one service';
    ...
    servOut.textContent=selected.map(function(s){return s.name;}).join(' + ')+(selected.length>1?' — combined project':'');
